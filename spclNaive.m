function [clusters, evalues, evectors] = spclNaive(data, nbclusters, varargin)
%
...
if ~iscell(data)
    error('data are not multi-view data, the input must have to be a cell whose each element is corresponding featue from one view');
else
    nbsamples = size(data{1},2);
    wmat = zeros(nbsamples);
        
    for j = 1:numel(data)
        twmat = zeros(nbsamples);
        dataOne = data{j}'; %dataOne means data from one view, it is nbsamples*dim matrix;
        
        percent = 0.25;
        params = determineSigma(dataOne, 1, percent);
        %params = 2.5;
        
        for i = 1: nbsamples - 1
            twmat(i, i + 1: end) = feval(func, repmat(dataOne(i, :), nbsamples - i, 1), dataOne(i + 1: end,:), params);
        end
